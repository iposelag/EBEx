...
This function trains a Random Forest model using the `parsnip` and
`workflows` packages. It performs hyperparameter tuning using Bayesian
optimization with the `tune` package, evaluates the model using
resampling, and saves the fitted model and resampling results as bundles
