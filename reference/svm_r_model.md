...
This function trains a Support Vector Machine with Radial Basis Function
kernel using the `parsnip` and `workflows` packages. It performs
hyperparameter tuning using Bayesian optimization with the `tune`
package, evaluates the model using resampling, and saves the fitted
model and resampling results as bundles in a specified directory.
